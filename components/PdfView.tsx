...
import { FileText, Lock, ArrowLeft, Crown, Star, CheckCircle, AlertCircle, Globe, Maximize, Layers, HelpCircle, Minus, Plus, Volume2, Square, Zap, RefreshCw } from 'lucide-react';
...
export const PdfView: React.FC<Props> = ({ 
  ...
  const handleGlobeClick = async () => {
      ...
      const streamKey = (classLevel === '11' || classLevel === '12') && stream ? `-${stream}` : '';
      const key = `nst_content_${board}_${classLevel}${streamKey}_${subject.name}_${chapter.id}`;
      const modeKey = syllabusMode === 'SCHOOL' ? 'schoolPremiumNotesHtml_HI' : 'competitionPremiumNotesHtml_HI';
      
      if (contentData && contentData[modeKey]) {
          // Exists locally/fetched
          setActiveLang('HINDI');
      } else {
          // GENERATE
          setAlertConfig({ isOpen: true, message: "Generating Hindi Translation with AI... Please wait." });
          try {
              const translated = await translateToHindi(activePdf);
              const updatedData = { ...contentData, [modeKey]: translated };
              
              setContentData(updatedData);
              await storage.setItem(key, updatedData);
              await saveChapterData(key, updatedData); // Sync to Firebase
              
              setActiveLang('HINDI');
              setAlertConfig({ isOpen: true, message: "Translation Complete! Saved for future." });
          } catch (e) {
              console.error(e);
       ...
       <div className="sticky top-0 z-20 bg-white border-b border-slate-100 shadow-sm p-4 flex items-center gap-3">
           ...
           {activePdf && !activePdf.startsWith('http') && (
               <button 
                   ...
                   className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm border transition-all mr-2 ${activeLang === 'HINDI' ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}
               >
                   <Globe size={12} />
                   {activeLang === 'HINDI' ? 'HINDI' : 'ENG'}
               </button>
           )}
                   ...
                   {activePdf.startsWith('http') ? (
                       ...
                       >
                           ...
                           {(() => {
                               ...
                               // 1. Check for AI Generated Hindi
                               const modeKey = syllabusMode === 'SCHOOL' ? 'schoolPremiumNotesHtml_HI' : 'competitionPremiumNotesHtml_HI';
                               if (activeLang === 'HINDI' && contentData && contentData[modeKey]) {
                                   contentToRender = contentData[modeKey];
                               }
                               // 2. Check for Legacy Split
                               else if (activePdf.includes('|||')) {
