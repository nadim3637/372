...
import { useInteractiveAudio } from '../hooks/useInteractiveAudio';
...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter, isDarkMode, onToggleDarkMode }) => {
  ...
  const { playButtonAudio, toggleAudio, isAudioEnabled, availableVoices, setVoice, currentVoiceURI } = useInteractiveAudio();
              ...
              <div className="pb-24">
                ...
                <div className="space-y-4 px-1">
                    ...
                    <div className="flex justify-end mb-2">
                        <button 
                            onClick={toggleAudio}
                            className="flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm transition-all bg-white text-slate-600 border border-slate-200"
                        >
                            Audio Guide {isAudioEnabled ? 'ON' : 'OFF'}
                        </button>
                    </div>

                    {/* 1. STUDY TIMER */}
                    <DashboardSectionWrapper id="hero_timer" label="Study Timer">
                        <StudyTimer onComplete={(secs) => showAlert(`Focus Session Complete! You studied for ${secs/60} mins.`, 'SUCCESS')} />
                    </DashboardSectionWrapper>

                    {/* 2. AI PERSONAL TUTOR HERO CARD */}
                    {settings?.isAiEnabled !== false && (
                    <DashboardSectionWrapper id="hero_ai_tutor" label="AI Tutor Hero">
                        <div 
                            onClick={() => {
                                playButtonAudio('ai_tutor_hero', "I am your personal AI Tutor. Ask me anything about your studies!");
                                onTabChange('AI_CHAT');
                            }}
                            className="relative h-40 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-[32px] p-6 shadow-xl overflow-hidden cursor-pointer group border-4 border-white/20"
                        >
                            <div className="absolute top-0 right-0 w-48 h-48 bg-white/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 group-hover:scale-110 transition-transform"></div>
                            
                            <div className="relative z-10 flex flex-col justify-center h-full">
                                <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1 rounded-full w-fit mb-3 border border-white/20">
                                    <Sparkles size={12} className="text-yellow-300 animate-pulse" />
                                    <span className="text-[10px] font-black text-white uppercase tracking-widest">AI Personal Tutor</span>
                                </div>
                                <h2 className="text-2xl font-black text-white leading-tight mb-1">
                                    "Ask anything – <br/>AI will teach you"
                                </h2>
                                <div className="flex items-center gap-2 mt-2">
                                    <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse"></div>
                                    <span className="text-[10px] font-bold text-indigo-100 uppercase tracking-wider">Online & Ready</span>
                                </div>
                            </div>
                            
                            <div className="absolute bottom-4 right-4 bg-white text-indigo-600 p-3 rounded-2xl shadow-lg group-hover:scale-110 transition-transform">
                    ...
                    {/* 3. CONTINUE LEARNING (Last Topic) */}
                    <DashboardSectionWrapper id="continue_learning" label="Continue Learning">
                        {user.usageHistory && user.usageHistory.length > 0 ? (
                            <button 
                                onClick={() => {
                                    const last = user.usageHistory![0];
                                    if (last.type === 'VIDEO' || last.type === 'PDF' || last.type === 'MCQ') {
                                        onTabChange('HISTORY');
                                    }
                                }}
                                className="w-full bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between group hover:border-blue-200 transition-colors"
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="text-left">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Continue Learning</p>
                                        <p className="font-bold text-slate-800 line-clamp-1">{user.usageHistory[0].itemTitle}</p>
                                    </div>
                                </div>
                            ...
                            <div className="bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-200 text-center">
                                <p className="text-xs text-slate-400 font-bold">Start studying to track progress!</p>
                            </div>
                        )}
                    </DashboardSectionWrapper>

                    {/* 4. DAILY CHALLENGE (Quick Start) */}
                    <DashboardSectionWrapper id="quick_challenge" label="Quick Challenge">
                        <button 
                            onClick={() => startAutoChallenge('DAILY')}
                            className="w-full bg-gradient-to-r from-orange-500 to-pink-500 p-1 rounded-2xl shadow-lg group active:scale-95 transition-transform"
                        >
                            <div className="bg-white rounded-[14px] p-3 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                        <h4 className="font-black text-slate-800 text-sm">Daily Challenge</h4>
                                        <p className="text-[10px] text-slate-500 font-bold">Win Rewards • 15 Mins</p>
                                    </div>
                                </div>
                                <div className="bg-slate-900 text-white px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider group-hover:bg-orange-600 transition-colors">
                                    Start
                                </div>
                            </div>
                        </button>
                ...
                {/* --- LAYER 2: SMART SLIDE (SWIPE UP) --- */}
                <SmartSlide onTourClick={() => {
                    playButtonAudio('ai_tour', "Welcome to the AI Tour. Let me show you around.");
                }}>
                  <div className="space-y-4">
                      ...
                      {/* --- LAYER 3: POWER USER ZONE (SERVICES GRID) --- */}
                      <DashboardSectionWrapper id="services_grid" label="Power Zone">
                          <h3 className="font-bold text-slate-800 mb-3 flex items-center gap-2 px-1">
                              <Layout size={18} /> Power Zone
                          </h3>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
                <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        <div className="h-px bg-slate-100 my-2"></div>
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">Voice Assistant</label>
                            <select 
